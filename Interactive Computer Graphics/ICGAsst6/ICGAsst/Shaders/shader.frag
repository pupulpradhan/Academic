...
void main()
{
    ...
    vec3 ambientColor = vec3(intensity * vec3(0.0,0.0,0.0));
    vec4 envLight = texture( env, reflectVec );
    vec3 envLightSampled = vec3(envLight) * vec3(0.5,0.5,0.5);
    vec3 clr = vec3(diffuseColor + specularColor + envLightSampled);
    color = vec4(clr,1);
}
