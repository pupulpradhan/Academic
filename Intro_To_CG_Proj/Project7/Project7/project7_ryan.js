...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	//add gravity
	for(i=0; i<positions.length; i++){
		var g_force = gravity.mul(particleMass);
		forces[i].inc(g_force);
	}
	//add spring forces
	for(s=0; s<springs.length; s++){
		//retrive spring values
		var i=springs[s].p0;
		var j=springs[s].p1;
		var rest = springs[s].rest;
		//calculate intermediates
		var pji = positions[j].sub(positions[i]);
		var l = pji.len();
	    var d_hat = pji.div(l);
		var l_dot = (velocities[j].sub(velocities[i])).dot(d_hat); 
		//calculate total spring force
		var fs = d_hat.mul(stiffness*(l - rest));
		var fd = d_hat.mul(damping*l_dot);
		var fds = fd.add(fs);
