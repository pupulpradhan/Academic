...
vec3 Shade( Material mtl, vec3 position, vec3 normal, vec3 view )
{
	...
	for ( int i=0; i<NUM_LIGHTS; ++i ) {
		...
		bool isShadowed = dot(normal, r.dir)<0.0 || IntersectRay( h, r );
		if (! isShadowed)
		{
			color += lights[i].intensity * ((max(0.0, dot(normalize(normal), r.dir)) * mtl.k_d + mtl.k_s * pow(max(0.0, dot(normalize(normal), normalize(r.dir + view))), mtl.n)));
		}
	}
	return color;
}
...
bool IntersectRay( inout HitInfo hit, Ray ray )
{
	...
	for ( int i=0; i<NUM_SPHERES; ++i ) {
		...
		if(hitCheck >= 0.0)
		{
			...
			if(currentHit > bias && currentHit < hit.t)
			{
				foundHit = true;
				hit.t = currentHit;
				hit.position = ray.pos + (currentHit * ray.dir);
