...
var StateController = function ( dispParams ) {
	...
	function computeMovement( x, y, previousPosition ) {
		...
		var position =  new THREE.Vector2(x-previousPosition.x,previousPosition.y-y);
		previousPosition.set(x,y);
		return position;
	...
	function updateModelParams( e, movement ) {
		...
		if ( e.shiftKey && ! ctrlKey ) {
			...
			// XY translation
			_this.state.modelTranslation.set(movement.x, movement.y	, 0);

		} else if ( ! e.shiftKey && ctrlKey ) {

			// Z translation
			_this.state.modelTranslation.set(0,0,movement.y);

		} else {

			//Rotation
			_this.state.modelRotation.x -= movement.x;
			_this.state.modelRotation.y += movement.y;
	...
	function updateViewPosition( e, movement ) {
		...
		if ( ! ctrlKey ) {
			...
			// XY translation
			_this.state.viewerPosition.set(movement.x, movement.y , dispParams.distanceScreenViewer);

		} else {

			// Z translation
			_this.state.viewerPosition.set(0, 0 , movement.y - dispParams.distanceScreenViewer);
	...
	function updateViewTarget( e, movement ) {
		...
		if ( ! ctrlKey ) {
			...
			// XY translation
			_this.state.viewerTarget.set(movement.x, movement.y , 0);

		} else {

			// Z translation
			_this.state.viewerTarget.set(0, 0 , movement.y);
	...
	function updateProjectionParams( e, movement ) {
		...
		_this.state.clipNear.set(0, movement.y , 0);
		_this.state.clipNear.clamp(new THREE.Vector3(0,1,0), new THREE.Vector3(10000,10000,10000));
